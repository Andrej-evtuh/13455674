...
def inPolygon(x, y, xp, yp):
    ...
    # check: is polygon ?
    if len(xp) < 3 or not len(xp) == len(yp):
        raise ValueError('In polygon must be at least three points')
    ...
    if c == 0:
        ...
        raise ValueError('A very specific bad thing happened... Andreas ass is burning')


print(inPolygon(4, 2, (5, 5,5), (0, 4,5)))

import matplotlib.pyplot as plt

x = 0.0
y = 4.0
plt.plot([5, 5, 2], [0, 4, 4], linewidth=2.0)
plt.plot([4], [2], 'ro')
plt.axis([0, 12, 0, 10])
plt.show()
...
if y < getY(x, k, b):
    print(y, getY(x, k, b), "Точка под прямой.")
...
if y == getY(x, k, b):
    print(y, getY(x, k, b), "Точка на прямой.")
...
if y > getY(x, k, b):
    print(y, getY(x, k, b), "Точка над прямой.")
'''
